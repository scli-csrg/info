...
# calculate forensic similarity in a tensorflow session. use this if you will be calculating forensic similarity
# multiple times, and do not want to create a tensorflow session each time

def calculate_forensic_similarity_insession(X1, X2, patch_size, sess, compare_output, tf_x1, tf_x2, MISL_phase,
                                            batch_size=48, quiet=False):
    ...
    for ix in tqdm(batch(range(len(X1)), batch_size), total=int(np.ceil(len(X1) / batch_size)),
        ...
        sim_batch = softmax(result)[:,
                    1]  # second value is similarity (first value/0th index is dissimilarity and not used)
...
def calculate_forensic_similarity(X1, X2, f_pretrained_weights, patch_size, batch_size=48, quiet=False):
    ...
    with tf.Session() as sess:
        ...
        similarity = calculate_forensic_similarity_insession(X1, X2, patch_size, sess, compare_output,
                                                             tf_x1, tf_x2, MISL_phase, batch_size=batch_size,
                                                             quiet=quiet)
